...
def take_to_session(username: str, user_questions: List, user_time: int, current_number: int, current_subject: str) -> str:
    lock.acquire(True)
    cursor.execute("CREATE TABLE IF NOT EXISTS CBT_SESSION(USERNAME TEXT, USER_QUESTIONS TEXT, USER_TIME INTEGER, STATUS TEXT, CURRENT_NUMBER INTEGER, CURRENT_SUBJECT TEXT)")
    # exist = None
    ...
    if len(user_fetch):
        cursor.execute(f"UPDATE CBT_SESSION SET USER_QUESTIONS=?, STATUS=?, USER_TIME=?, CURRENT_NUMBER=?, CURRENT_SUBJECT=? WHERE USERNAME=?", (user_dict, current, user_time, current_number, current_subject, username))
    else:
        cursor.execute(f"INSERT INTO CBT_SESSION (USERNAME , USER_QUESTIONS , USER_TIME, STATUS, CURRENT_NUMBER, CURRENT_SUBJECT) VALUES(?, ?, ?, ?, ?, ?)", (username, user_dict, user_time, current, current_number, current_subject))
...
def exam_settings(username: str, selected_subject: str) -> Tuple:
    ...
    cursor.execute(f"SELECT * FROM {setting_table} WHERE SUBJECT=?", (selected_subject,))
...
def check_query(username: str) -> Tuple:
    ...
    if username != "":
        initial_obj = cursor.execute(f"SELECT * FROM USERS WHERE username=?", (username,))
    else:
...
def techers_changes(new_list: List, table_name: str) -> bool:
    ...
    try:
        for detail in new_list:
            ...
            # print(column_name)
            cursor.execute(f"UPDATE {table_name} SET {column_name}=? WHERE {target_column}=?", (detail[0], detail[2]))
        conn.commit()
        print("WELDONE")
    except IndexError:
...
def grant_permission(_session: str, enrollment: str, upload_result: str, upload_attendance: str) -> str:
    
    if fetch_table_details("Admin_permissions") == []:
        cursor.execute("INSERT INTO Admin_permissions (SESSION_ID, SESSION, ENROLLMENT, UPLOADING_RESULTS, UPLOADING_ATTENDANCE) VALUES(?, ?, ?, ?, ?)", (00, _session, enrollment, upload_result, upload_attendance))
        conn.commit()
    else:
        session_id = fetch_table_details("Admin_permissions")[0][0]
        cursor.execute(f"UPDATE Admin_permissions SET SESSION=?, ENROLLMENT=?, UPLOADING_RESULTS=?, UPLOADING_ATTENDANCE=? WHERE SESSION_ID=?", (_session, enrollment, upload_result, upload_attendance, session_id))
        conn.commit()

        
    return "PERMISSION DONE"

# my_list = [('STUDENT05', 2), ('STUDENT54', 3)]
# # [('STUDENT05', 'USER TYPE ', 2, '2', 'ID '), ('STUDENT54', 'USER TYPE ', 3, '3', 'ID ')]
# cursor.executemany("UPDATE USERS SET USER_TYPE=? WHERE ID=?", my_list)
# conn.commit()
...
def moderate_scores(value: int, subject: str, class_name: str, test_type: str,username: str = "*"):
    ...
    if username == "*":
        ...
        cursor.execute(f"SELECT USERNAME, {test_type} FROM {subject_table} WHERE USERNAME=?", (username,))
        tobe_moderate = cursor.fetchall()
    ...
    for user_detail in tobe_moderate:
        ...
        if overall_score < max_value:
            cursor.execute(f"UPDATE {subject_table} SET {test_type}=? WHERE USERNAME=?", (overall_score, user_detail[0]))
            conn.commit()
        else:
            cursor.execute(f"UPDATE {subject_table} SET {test_type}=? WHERE USERNAME=?", (max_value, user_detail[0]))
            conn.commit()
...
def stb_score(username, test_type, subject, class_name):
    ...
    for user_detail in tobe_moderate:
        ...
        if user_score > max_score:
            cursor.execute(f"UPDATE {subject_table} SET {test_type}=? WHERE USERNAME=?", (max_score, user_detail[0]))
            conn.commit()
...
def make_report_sheet(class_name: str) -> bool:
    jss_2_table = [table for table in db_tables() if class_name.replace("_", " ") in table if "SETTINGS" and "DETAILS" not in table]
    jss_2_table.sort()
    ...
    my_school = School()
    # try:
    cursor.execute(f"""CREATE TABLE IF NOT EXISTS {class_name}_INFO_SHEET(id INTEGER PRIMARY KEY, USERNAME TEXT UNIQUE, FIRST_NAME TEXT, SURNAME TEXT, LAST_NAME TEXT, REGISTER_NUMBER NUMBER, MAX_ATTENDANCE NUMBER, 
    ...
    try:
        for std_info in fetch_by_class(class_name):
            ...
            empt = ""
            cursor.execute(f"""INSERT INTO {class_name}_INFO_SHEET (USERNAME, FIRST_NAME, SURNAME,
    ...
    for offer_subj in offered_subjects:
        ...
        cursor.execute(f"SELECT username, first_name, surname, middle_name, FIRST_CA, SECOND_CA, EXAM, PPT FROM {offer_subj}")
    ...
    for std_info in fetch_details:
        ...
        cursor.execute(f"""UPDATE {class_name}_INFO_SHEET SET MAX_ATTENDANCE=?,  
        NUMBER_OF_PRESENT_DAYS=?, NUMBER_OF_STUDENT=?, TERM_ENDED=?, NUMBER_OF_ABSENT_DAYS=? WHERE REGISTER_NUMBER=?
        """, (std_info[5], std_info[6], len(fetch_details), my_school.term_ended, std_info[7], reg_number))
      
    try:
        cursor.execute(f"ALTER TABLE {class_name}_SCORE_SHEET ADD COLUMN TOTAL NUMBER;")
        cursor.execute(f"ALTER TABLE {class_name}_SCORE_SHEET ADD COLUMN POSITION TEXT;")
    except sqlite3.OperationalError:
        pass
    ...
    try:
        for detail in fetch_class:
            cursor.execute(f"INSERT INTO {class_name}_SCORE_SHEET (username, first_name, surname, MIDDLE_NAME) VALUES(?, ?, ?, ?)", (detail[-2], detail[1], detail[2], detail[3]))
    except sqlite3.IntegrityError:
        pass
    # FETCHING THE SCORES TO THE BROAD SHEET
 
    for indiv_subj in compiled_results:
        cursor.execute(f"UPDATE {class_name}_SCORE_SHEET SET {indiv_subj[-1]}_FIRST_CA=?, {indiv_subj[-1]}_SECOND_CA=?, {indiv_subj[-1]}_EXAM=? WHERE USERNAME=?", (indiv_subj[-4], indiv_subj[-3], indiv_subj[-2], indiv_subj[0]))
    ...
    for std in fetch_table_details(class_name + "_SCORE_SHEET"):
        ...
        cursor.execute(f"UPDATE {class_name}_SCORE_SHEET SET TOTAL=? WHERE USERNAME=?", (make_total(scores), std[1]))
    ...
    for score in std_scr:
        ...
        position += 1
        cursor.execute(f"UPDATE {class_name}_SCORE_SHEET SET POSITION=? WHERE TOTAL=?", (real_position, score))
...
def process_results(class_name: str) -> List:
    jss_2_table = [table for table in db_tables() if class_name.replace("_", " ") in table if "SETTINGS" not in table]
    jss_2_table.sort()
    ...
    for std_details in fetched_class:
        ...
        for table in new_tables:
            # print(table)
            cursor.execute(f"SELECT * FROM {table.replace(' ', '_')} WHERE USERNAME=?", (username,))
        ...
        cursor.execute(f"SELECT TOTAL, POSITION FROM {class_name}_SCORE_SHEET WHERE USERNAME=?", (username,))
...
class User():
    ...
    def cheched_written(self, selected_subject: str) -> bool:
        ...
        cursor.execute(f"SELECT EXAM_TYPE from {user_setting_table} WHERE SUBJECT=?", (selected_subject,))
        try:
        ...
        cursor.execute(f"SELECT {exam_type} FROM {user_class_score} WHERE username=?", (self.username,))
    ...
    def check_password(self, password: str) -> str:
        cursor.execute("SELECT user_Password FROM USERS WHERE username=?", (self.username,))
        fetched_user = cursor.fetchone()
    ...
    def verify_subject(self, selected_subject: str) -> bool:
        ...
        cursor.execute(f"SELECT SUBJECT FROM {setting_table} WHERE SUBJECT=?", (selected_subject,))
        collected_subject = cursor.fetchone()
    ...
    def save_score(self, selected_subject: str, questions: List) -> str:
        ...
        cursor.execute(f"UPDATE {user_subject_score} SET {exam_type}=? WHERE username=?", (user_score, self.username))
        conn.commit()
    ...
    def upload_image(self):
        ...
        if 'image' in request.files:
            ...
            if image_file.filename != '':
                ...
                cursor.execute("UPDATE USERS SET img_upload = ? WHERE username = ?", (encoded_image, self.username))
...
def get_results(class_name: str, username: str = "*") -> List:
    ...
    if username == '*':
        ...
        cursor.execute(f"SELECT MAX_ATTENDANCE, NUMBER_OF_PRESENT_DAYS, NUMBER_OF_ABSENT_DAYS, GENDER, NUMBER_OF_STUDENT FROM {info_sheet} WHERE REGISTER_NUMBER=?", (reg_num,))
...
class Admin():
    def assign_to(self, username: str, teachers_name: str, role_assigned: str, assigned_class: str):
        cursor.execute("CREATE TABLE IF NOT EXISTS ROLE_ASSIGNMENT(USERNAME TEXT,NAME TEXT, ROLE TEXT, ASSIGNED_CLASS TEXT, ROLE_NAME TEXT)")
        role_id = role_assigned + assigned_class
        fetch_user = check_exist("ROLE_ASSIGNMENT", "ROLE_NAME", role_id)
        print(fetch_user)
        if fetch_user == []:
            cursor.execute("INSERT INTO ROLE_ASSIGNMENT VALUES(?, ?, ?, ?, ?)", (username, teachers_name, role_assigned, assigned_class, role_id))
            conn.commit()
        else:
            cursor.execute("UPDATE ROLE_ASSIGNMENT SET NAME=?, USERNAME=? WHERE ROLE_NAME=?", (teachers_name, username, fetch_user[0][-1]))
            conn.commit()
    
    def fetch_report_card(self, class_name: str = "JSS 2", username: str = "*") -> List:
        jss_2_table = [table for table in db_tables() if class_name in table if "SETTINGS" and "DETAILS" not in table]
        jss_2_table.sort()
        empt = []
        for table in jss_2_table:
            ...
            if "username" in table_columns(table_name=table):
                empt2 = []
                if username == "*":
                    cursor.execute(f"SELECT first_name, surname, middle_name, reg_number, FIRST_CA, SECOND_CA, EXAM FROM {table}",)
                else:
                    cursor.execute(f"SELECT first_name, surname, middle_name, reg_number, FIRST_CA, SECOND_CA, EXAM FROM {table} WHERE username=?",(username,))

                fetch = list(cursor.fetchall())
                print(table.replace("JSS_2_SCORE", ""))
                empt2.append(table.replace("JSS_2_SCORES", "").replace("_", " ").strip())
                empt2.append(fetch)
                empt.append(empt2)
            else:
                pass
