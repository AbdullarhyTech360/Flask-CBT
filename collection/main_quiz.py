...
from quiz_logic import (Admin, check_exist, db_tables, exam_settings,
    fetch_table_details, cursor, conn, grant_permission, take_to_session, remove_trash, scheduled_subjects, store_option, table_columns,
    techers_changes, User, get_results)
...
def report_gate():
    if request.method == "GET":
        ...
        if username != "":
            print(username == "")
            check_user = check_exist(class_name + "_DETAILS", "USERNAME", username)
            print(check_user)
            return jsonify({"check_user": check_user})
        return  jsonify({"check_user": None})
        # class_name = ""
        
@app.route('/admin/report-card', methods=["POST", "GET"])
def report_card():
    # print(results)
    username = session["username"]
    class_name = session["class_name"]

    results = get_results(class_name, username)
    # results = [[['AGRIC SCIENCE', 19, 'FATIMA', 'SANI', 'UMAR', '031', 'JS23031', 10, 5, 16, 10, 26, 41, ['E', 'Weak']], ['ARABIC LANGUAGE', 19, 'FATIMA', 'SANI', 'UMAR', '031', 'JS23031', 18, 5, 10, 20, 30, 53, ['C', 'Good']], ['BASIC STUDIES', 19, 'FATIMA', 'SANI', 'UMAR', '031', 'JS23031', 13, 6, 15, 5, 20, 39, ['E', 'Weak']], ['BUSINESS STUDIES', 19, 'FATIMA', 'SANI', 'UMAR', '031', 'JS23031', 10, 6, 17, 10, 27, 43, ['E', 'Weak']], ['CIVIC EDUCATION', 19, 'FATIMA', 'SANI', 'UMAR', '031', 'JS23031', 10, 10, 15, 11, 26, 46, ['D', 'Pass']], ['COMPUTER', 19, 'FATIMA', 'SANI', 'UMAR', '031', 'JS23031', 10, 7, 18, 10, 28, 45, ['D', 'Pass']], ['ENGLISH LANGUAGE', 19, 'FATIMA', 'SANI', 'UMAR', '031', 'JS23031', 10, 1, 15, '', 15, 26, ['F', 'Fail']], ['HAUSA LANGUAGE', 19, 'FATIMA', 'SANI', 'UMAR', '031', 'JS23031', 5, '', 10, '', 10, 15, ['F', 'Fail']], ['IRK', 19, 'FATIMA', 'SANI', 'UMAR', '031', 'JS23031', 10, 5, 13, 10, 23, 38, ['E', 'Weak']], ['MATHEMATICS', 19, 'FATIMA', 'SANI', 'UMAR', '031', 'JS23031', 10, 2, 10, 7, 17, 29, ['F', 'Fail']], ['SOCIAL STUDIES', 19, 'FATIMA', 'SANI', 'UMAR', '031', 'JS23031', 14, 1, 15, 15, 30, 45, ['F', 'Fail']], (410, '20th'), [None, None, None, 'FEMALE', 21, 'JSS 2']]]
    # results = [[['AGRIC SCIENCE', 25, 'SAUDAT', 'SULAIMAN', '', '032', 'JS24032', 15, 10, 16, 15, 31, 56, ['C', 'Good']], ['ARABIC LANGUAGE', 25, 'SAUDAT', 'SULAIMAN', '', '032', 'JS24032', 20, 10, 10, 26, 36, 66, ['B', 'Very Good']], ['BASIC STUDIES', 25, 'SAUDAT', 'SULAIMAN', '', '032', 'JS24032', 15, 10, 16, 32, 48, 73, ['A', 'Excellent']], ['BUSINESS STUDIES', 25, 'SAUDAT', 'SULAIMAN', '', '032', 'JS24032', 17, 13, 20, 16, 36, 66, ['B', 'Very Good']], ['CIVIC EDUCATION', 25, 'SAUDAT', 'SULAIMAN', '', '032', 'JS24032', 17, 15, 17, 15, 32, 64, ['B', 'Very Good']], ['COMPUTER', 25, 'SAUDAT', 'SULAIMAN', '', '032', 'JS24032', 12, 11, 12, 11, 23, 46, ['D', 'Pass']], ['ENGLISH LANGUAGE', 25, 'SAUDAT', 'SULAIMAN', '', '032', 'JS24032', 11, 15, 11, 10, 21, 47, ['D', 'Pass']], ['HAUSA LANGUAGE', 25, 'SAUDAT', 'SULAIMAN', '', '032', 'JS24032', 5, '', 10, '', 10, 15, ['F', 'Fail']], ['IRK', 25, 'SAUDAT', 'SULAIMAN', '', '032', 'JS24032', 18, 5, 16, 25, 41, 64, ['B', 'Very Good']], ['MATHEMATICS', 25, 'SAUDAT', 'SULAIMAN', '', '032', 'JS24032', 13, 10, 10, 11, 21, 44, ['E', 'Weak']], ['SOCIAL STUDIES', 25, 'SAUDAT', 'SULAIMAN', '', '032', 'JS24032', 14, 12, 14, 4, 18, 44, ['E', 'WEAK']], (575, '14th'), [None, None, None, 'FEMALE', 25, 'JSS 1']]]
    # print(len(results))

    return render_template("report-card.html", term_ended = current_school.term_ended, class_name = class_name.replace("_", " "), _session = current_school._session, term = current_school.current_term, student_results = results, username = username, school_name = current_school, term_begin = current_school.term_begin)
...
def admin():
    ...
    curs.execute("CREATE TABLE IF NOT EXISTS Admin_permissions(SESSION_ID TEXT, SESSION NUMBER, ENROLLMENT TEXT, UPLOADING_RESULTS TEXT, UPLOADING_ATTENDANCE TEXT)")
    classes = ["PRIMARY_4", "PRIMARY_5", "JSS_1", "JSS_2", "JSS_3"]
    for clas in classes:
        curs.execute(f"CREATE TABLE IF NOT EXISTS {clas}_SETTINGS(CLASS TEXT, SUBJECT TEXT, EXAM_TYPE, DURATION NUMBER, NUMBER_OF_QUESTIONS NUMBER, MARK_PER_QUESTION NUMBER)")
    general_db.commit()
    general_db.close()
    if request.method == "GET":
        if "admin" in session:
...
def staff():
    if request.method == "GET":
        if "teacher" in session:
            ...
            print(role_table)
            return render_template('teachers-page.html', role_details = role_table, teachers_name = teachers_name)
        else: return redirect(url_for("loginForm"))
    else:
        if "scores" in request.form:
            obtained_string = request.form["scores"]
            # print(obtained_string)
            new_list = obtained_string.replace("(", "").replace(")", "").replace("'", "").replace("'", "").split(', ')
            ...
            allowable_column = student_data_columns.index(current_school.current_test.upper())
            ...
            # print(informations)
            return render_template('input-scores.html', tables_column = student_data_columns, student_list = informations)
            ...
            student_data = remove_trash(table_name) # fetch_table_details(table_name)
            ...
            return render_template('input-scores.html', student_list = informations, tables_column = student_data_columns, attend = "attendance")
...
def exams_settings():
    if "admin" in session:
        if request.method == "POST":
            ...
            initial_obj = curs.execute(f"SELECT * FROM {selected_class}_SETTINGS WHERE SUBJECT=?", (selcted_subject,))
            initial_state = initial_obj.fetchall()
            if initial_state:
                curs.execute(f"UPDATE {selected_class}_SETTINGS SET DURATION=?, NUMBER_OF_QUESTIONS=?, EXAM_TYPE=?, MARK_PER_QUESTION=? WHERE SUBJECT=?", (duration, number_of_questions, exam_type, mark_per_question, selcted_subject))
            else:
                curs.execute(f"INSERT INTO {selected_class}_SETTINGS VALUES(?, ?, ?, ?, ?, ?)", (selected_class, selcted_subject, exam_type, duration, number_of_questions, mark_per_question))
            general_db.commit()
            general_db.close()
            return render_template("examsSettings.html",success = "Changes had been made successfully", subjects = current_school.overall_subjects, shool_classes = current_school.classes, current_test = current_school.current_test, school_test = current_school.tests)
        else:
            return render_template("examsSettings.html", subjects = current_school.overall_subjects, shool_classes = current_school.classes, current_test = current_school.current_test, school_test = current_school.tests)
    else:
        return redirect(url_for("loginForm"))
...
def assign_role():
    if "admin" in session:
        ...
        for detail in teacher_details:
            complete_name = detail[1] + " " + detail[2] + " " + detail[3] + " - " + detail[-2]
        ...
        if request.method == "GET":
            try:
                ...
                print("here except")
                return render_template("role.html", teachers_names = teachers_names, subjects = current_school.overall_subjects, shool_classes = current_school.classes)
            return render_template("role.html", teachers_names = teachers_names, role_details= role_details, subjects = current_school.overall_subjects, shool_classes = current_school.classes)
        else:
...
def making_query():
    ...
    if request.method == "POST":
        ...
        print(tables_column)
        return render_template("makingQuery.html", database_table = database_table, selected_table = selected_table, table_columns = tables_column, informations = fetch_table_details(selected_table))
...
def quiz():
    ...
    try:
        ...
        # print(status)
        # print(status)
        # print(bytes(user_image), "User image taken")
        ...
        print(len(questions))
        print(session["exams_settings"][4])
        ...
        for user_question in session["user_questions"]:
            new_options = sample(user_question[1], 4)
        ...
        full_name = Student.first_name + ' ' + Student.surname + ' ' + Student.middle_name
        ...
        return render_template('basec.html', exam_time = exam_time, question= user_questions[quest_index][0], quest_inst = user_questions[quest_index][-1], name = full_name, regNo = regNo, subject = subject, remaining_questions = remaining_questions, user_questions = user_questions, username = username, number_of_quest = len(user_questions),
                                options = user_questions[quest_index][1], answer = user_questions[quest_index][2], quest_number = quest_index + 1, number_of_questions = len(user_questions), user_class = user_class.upper(), selected_option = user_questions[quest_index][3], user_image = user_image, shaded_questions = shaded_questions)
    except KeyError:
        return redirect(url_for("loginForm"))
...
def cbt_session():
    if request.method == "POST":
        ...
        lock.acquire(True)
        cursor.execute(f"UPDATE CBT_SESSION SET USER_TIME=? WHERE USERNAME=?", (session_time, username))
        conn.commit()
        lock.release()
...
def load_questions():
    ...
    if request.method == "GET":
        quest_index = int(request.args.get('quest_index'))
        ...
        if next_index < len(user_questions):
            ...
            return jsonify({'question': new_question, 'show_selected': show_selected, "number_of_quest" : len(user_questions)})
        else:
            return jsonify({'message': 'End of question'})

@app.route("/load_pquestions", methods=["GET"])
def load_pquestions():
    user_questions = session.get("user_questions")
    try:
        if request.method == "GET":
...
def rem_quest():
    ...
    # print(total_time)
    take_to_session(session["username"], user_questions, total_time, quest_index + 1, session["selected_subject"])
    session.modified = True
    ...
    if request.method == "GET":
        ...
        return jsonify({"alert": "Remaining questions recieved successflly!", "remQuest": remaining_questions})

@app.route("/submit", methods = ["POST"])
def submit(): 
    confirm = 'Are you sure you want to submit? Once you submitted you cannot come back again'
...
def ok_submit():
    ...
    lock.acquire(True)
    cursor.execute(f"UPDATE CBT_SESSION SET USER_QUESTIONS=?, STATUS=? WHERE USERNAME=?", (str(session["user_questions"]), "SUBMITTED", session["username"]))
    conn.commit()
    lock.release()
    return redirect(url_for("loginForm"))
...
def handle_login():
    if request.method == "POST":
        ...
        if New_user.is_registered() == True:
            if New_user.check_user_type() == "STUDENT":
                if New_user.check_password(password) == "CORRECT":
                    if New_user.verify_subject(subject) == True:
                        if New_user.cheched_written(subject) == False:
                            open_subject = subject.lower() + "_" + New_user.user_class.lower() + '.json'
                            ...
                            with open(open_subject, encoding="utf-8") as f_obj:
                                ...
                                session["exams_settings"] = exam_settings(username, subject)
                                return jsonify({"quiz": "quiz"})
                        else:
                            return jsonify({"error": "You have already taken this test/exam"})
                    else:
            ...
            return jsonify({"error": "Sorry you have not registered or make sure your username is correct"})
    ...
    if request.method == "POST":
        try:
            ...
            if New_user.is_registered() == True:
                ...
                return render_template('imageUpload.html', image = encoded_image, success="Image uploaded successfully")
            else: return render_template('imageUpload.html', error = "Incorrect username")
        except TypeError:
...
def process_registeration():
    if request.method == "POST":
        ...
        username = collected_user_class[0] + user_type[0] + str(initial_year)[-2:] + str(reg_number)
        ...
        if create_password == confirm_password and fetch_reg_numbers == None:
            ...
            if user_type == "STUDENT" and reg_number != '':
                ...
                try:
                    teachers_list = gen_cursor.execute(f"SELECT * FROM USERS WHERE user_type = ?", (user_type,))
                    teachers_list = teachers_list.fetchall()
                    print(teachers_list)
                    username = collected_user_class[0] + user_type[0] + "00"+str(len(teachers_list) + 1)
                    
                    
                except sqlite3.OperationalError:
                    username = collected_user_class[0] + user_type[0] + "001"

                gen_cursor.execute(f"INSERT INTO USERS (first_name, surname, middle_name, gender, dob, img_upload,user_type, user_class, username, user_Password) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", 
                (first_name.upper(), surname.upper(), middle_name.upper(), gender.upper(), dob, image_upload,user_type, user_class.upper(), username, create_password))
                gen_cursor.execute(f"""CREATE TABLE IF NOT EXISTS TEACHERS_DETAILS(
                ...
                gen_cursor.execute(f"INSERT INTO TEACHERS_DETAILS (first_name, surname, middle_name, gender, dob, user_type, user_class, username, user_Password) VALUES( ?, ?, ?, ?, ?, ?, ?, ?, ?)", 
                    (first_name.upper(), surname.upper(), middle_name.upper(), gender.upper(), dob,user_type, user_class.upper(), username, create_password))
                gen_connect.commit()
                gen_connect.close()
                return jsonify({"success": f"Dear {first_name.upper()} {surname.upper()} {middle_name.upper()}, you have registered successfully. Your username is {username}"})
            else:
